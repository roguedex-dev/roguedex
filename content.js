...
function createPokemonCardDiv(cardclass, cardId, pokemon) {
    ...
    const opacitySlider = createOpacitySliderDiv(
        cardId,
        wrapperDivPositions[cardId].opacity,
        opacityRangeMin,
        opacityRangeMax
    )
    const typeEffectivenessHTML = createTypeEffectivenessWrapper(
...
function createCardsDiv(divId) {
    const newDiv = createWrapperDiv(divId)
    ...
    const divClass = divId === 'enemies' ? 'enemy-team' : 'allies-team'
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (
        ...
        const divId =
            message.type === 'UPDATE_ENEMIES_DIV' ? 'enemies' : 'allies'
            ...
            if (
                ...
                if (
                    ...
                    for (key in localStorage) {
                        if (
                            (slotId > 0 &&
                                key.includes(`sessionData${slotId}`)) ||
