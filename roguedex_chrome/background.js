...
// Merge two lists together while preventing duplicates
// > Use: Incoming API call for pokemon data is merged with exisiting weakness, resistance, and immunity data to aggregate data on ally's/enemy's team
// Code was gathered from: https://stackoverflow.com/questions/1584370/how-to-merge-two-arrays-in-javascript-and-de-duplicate-items
const mergeTwoLists = (allTypeEffectivenessList, newTypeEffectivenessList, 
  predicate = (allTypeEffectivenessList, newTypeEffectivenessList) => allTypeEffectivenessList === newTypeEffectivenessList
  ) => {
  const combinedTypeEffectivenessList = [...allTypeEffectivenessList] // copy to avoid side effects

  // add all items from newTypeEffectivenessList to copy into combinedTypeEffectivenessList if they're not already present
  newTypeEffectivenessList.forEach((newTypeEffectivenessItem) => (
    combinedTypeEffectivenessList.some((combinedTypeEffectivenessItem) => 
      predicate(newTypeEffectivenessItem, combinedTypeEffectivenessItem)) ? null : combinedTypeEffectivenessList.push(newTypeEffectivenessItem)
    )
  )
  return combinedTypeEffectivenessList;
}
...
function appendPokemonArrayToDiv(pokemonArray, arena, message) {
  let frontendPokemonArray = [
    // Functionality: Aggregate all of Team's Weaknesses, Resistances, and Immunities
    // > Important data is the aggregation of weaknesses, resistances, and immunities - Allows player to see coverage of ally/enemy team
    // > Dummy data was inserted as Value:0 in id, ivs, ability, and nature
    // Dev Note: Need to save a spot for incoming pokemon data - Index 0
    // > New pokemon data will be appended to the end of the list (After Index 0)
    {
      'id': 0,
      'typeEffectiveness': {
        'weaknesses': [], 
  ...
  pokemonArray.forEach((pokemon) => {
    ...
    getPokemonTypeEffectiveness(pokemonId).then((typeEffectiveness) => {
      ...
      // Team Aggregation of Weaknessess, Resistances, and Immunities in Index 0 of frontendPokemonArray
      // Merge the current list at index 0 with the new incoming list from the API call
      frontendPokemonArray[0]['typeEffectiveness']['weaknesses'] = mergeTwoLists(frontendPokemonArray[0]['typeEffectiveness']['weaknesses'], typeEffectiveness.weaknesses)
      frontendPokemonArray[0]['typeEffectiveness']['resistances'] = mergeTwoLists(frontendPokemonArray[0]['typeEffectiveness']['resistances'], typeEffectiveness.resistances)
      frontendPokemonArray[0]['typeEffectiveness']['immunities'] = mergeTwoLists(frontendPokemonArray[0]['typeEffectiveness']['immunities'], typeEffectiveness.immunities)
